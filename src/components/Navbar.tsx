...
const TextMenu = ({name, tclass, onClk=(() => {})}: Props) => (
  <li className={`text-ltext dark:text-dtext hover:text-lprimary dark:hover:text-dprimary ${tclass}`} onClick={onClk}>
    <a href={`#${name.toLowerCase()}`}>{name}</a>
  </li>
)

export const Button = (props: {name: string, onClick: () => void}) => (
  <button onClick={props.onClick} className='text-lprimary dark:text-dprimary font-semibold py-2 px-6 my-2 mx-2 border hover:text-dtext dark:hover:text-dtext border-lprimary dark:border-dprimary  rounded  hover:bg-lprimary hover:dark:bg-dprimary'>{props.name}</button>
)
...
export const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 w-full bg-lbg dark:bg-dbg'>
      <nav className='flex justify-between items-center h-24 max-w-[1240px] mx-auto px-8 ease-in-out duration-500'>
        ...
        <ul className='hidden md:flex'>
          {elements.map((e, i) => <TextMenu key={i} name={e} tclass='p-4'/>)}
        ...
        <ul className={`md:hidden fixed shadow-lg top-24 w-[60%] bg-lbg dark:bg-dbg h-full ease-in-out duration-300 ${nav ? 'right-0' : 'right-[-100%]'}`}>
          <ul className='flex flex-col items-center'>
            {elements.map((e, i) => (
              <TextMenu name={e} key={i} onClk={handleNav} tclass={`p-4 items-center ${i!==3 ? ' border-b border-dprimary' : ''}`}/>
            ))}
            <Button name='Resume' onClick={() => {}}/>
          </ul>
        </ul>
